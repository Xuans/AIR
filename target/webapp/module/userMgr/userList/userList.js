define([].concat(window.aweb.transformJsConfig([ "ctn", "ctn.divCtn", "component.foundationForm", "component.foundationForm.foundationInput", "component.foundationForm.foundationText", "ctn.foundationRowCtn", "ctn.modalCtn", "ctn.foundationFormCtn", "jquery.dataTables", "dataTables.fixedColumns.min", "component.dataTables", "component.foundationForm.foundationRadioGroup" ])).concat(window.aweb.transformCssConfig([ "AUI/css/layout.ctn.css", "foundation/css/foundation.css", "jQuery.dataTables/css/jquery.dataTables.css", "jQuery.dataTables/css/fixedColumns.dataTables.css" ])), function() {
    return function() {
        return function() {
            return function(auiCtx) {
                ...
                $AW.off(USER_LIST_REFRESH + NAMESPACE).on(USER_LIST_REFRESH + NAMESPACE, function(type) {
                    $.ajax({
                        type: "POST",
                        url: REFRESH_URL,
                        shelter: "正在加载数据，请稍候…",
                ...
                auiCtx = {
                    ...
                    attr: {
                        ...
                        foundationFormCtn75: {
                            desp: "Foundation表单容器75",
                            widgetName: "Foundation表单容器75",
                            name: "",
                            id: "foundationFormCtn75"
                        },
                        foundationRowCtn76: {
                            desp: "Foundation表单行容器76",
                            widgetName: "Foundation表单行容器76",
                            name: "",
                            id: "foundationRowCtn76"
                        },
                        newUserPw: {
                            desp: "新建用户密码",
                            widgetName: "新建用户密码",
 ...
 *
 */
                                $AW.trigger(app.router.USER_LIST_REFRESH);
                            },
                            times: 0,
                            clock: 1e3,
                            immediate: false,
                    ...
                    ajaxOption: {
                            ...
                            success: function(response) {
                                if (response.status) {
                                    if (response.type && response.type !== "AJAX") {
                                        ...
                                        app.alert("创建用户成功！");
                                        auiCtx.variables.modalCreSucCtn.show(null, {
                                            height: "250px",
                                            width: "400px"
                                        });
                                        auiCtx.variables.modalCreUserCtn.hide();
                                        auiCtx.variables.newUserPw.setValue(response.content.result);
                                        app.ajax({
                                            type: "POST",
                                            url: "##14917ABF0B9141F7BBFEF70BA9CFE851_URL##",
                                            success: function success(response) {
                                                if (response && response.status && response.content.result) {
                                                    auiCtx.variables.userListDataTables.refresh(response.content.result);
                                                } else {
                                                    app.alert(response && response.errorMsg || "用户列表加载失败！", app.alert.ERROR);
                                                }
                                            },
                        ...
                        _userListDataTables__editUserBtn_click_event_echo: {
                            type: "POST",
                            url: "./springmvc/user/editUser.do",
                            urlDivider: "/",
                            timeout: undefined,
                            ...
                            success: function(response) {
                                if (response.status) {
                                    if (response.type && response.type !== "AJAX") {
                                        ...
                                        auiCtx.eventCallback._userListDataTables__editUserBtn_click_event_echo(response);
                                    }
                                } else if (response.errorMsg) {
                                    app.alert(response.errorMsg, app.alert.ERROR);
                                }
                        ...
                        _userListDataTables__detailBtn_click_event_echo: {
                            type: "POST",
                            url: "./springmvc/user/listAllRoles.do",
                            urlDivider: "/",
                            timeout: undefined,
                            noAgreeBusData: true,
                            ajaxProcessData: true,
                    ...
                    eventCallback: {
                        ...
                        _userListDataTables__editUserBtn_click_event_echo: function(response) {
                    ...
                    delegateEvents: {
                        ...
                        "click #modalCreSucCtn [data-role='cancel']": function(e) {
                            aweb.debug && aweb.stepTo("userMgr-userList-事件配置-“创建用户成功弹窗”事件268");
                            auiCtx.variables.modalCreSucCtn.hide();
                        },
                        ...
                        "click #modalCreSucCtn [data-role='confirm']": function(e) {
                            aweb.debug && aweb.stepTo("userMgr-userList-事件配置-“创建用户成功弹窗”事件458");
                            auiCtx.variables.modalCreSucCtn.hide();
                        },
                        ...
                        "click #userListDataTables #createUserBtn": function(e) {
                            aweb.debug && aweb.stepTo("userMgr-userList-事件配置-创建用户事件");
                        ...
                        "click.event_echo #userListDataTables #editUserBtn": function(e) {
                            aweb.debug && aweb.stepTo("userMgr-userList-事件配置-修改用户信息回显");
                            $.ajax(auiCtx.ajaxOption._userListDataTables__editUserBtn_click_event_echo);
                        },
                    ...
                    auiCtxLoad: function(auiCtx, _$el, _scope, _handler) {
                        ...
                        variables.modalCreSucCtn = $AW.ctn.modalCtn($("#modalCreSucCtn", $el), configs.modalCreSucCtn, attr.modalCreSucCtn, $AW.css("ctn.modalCtn", css.modalCreSucCtn), auiCtx);
                        variables.foundationFormCtn75 = $AW.ctn.foundationFormCtn($("#foundationFormCtn75", $el), configs.foundationFormCtn75, attr.foundationFormCtn75, $AW.css("ctn.foundationFormCtn", css.foundationFormCtn75), auiCtx);
                        variables.foundationRowCtn76 = $AW.ctn.foundationRowCtn($("#foundationRowCtn76", $el), configs.foundationRowCtn76, attr.foundationRowCtn76, $AW.css("ctn.foundationRowCtn", css.foundationRowCtn76), auiCtx);
                        variables.newUserPw = $AW.component.foundationForm.foundationText($("#newUserPw", $el), configs.newUserPw, attr.newUserPw, $AW.css("component.foundationForm.foundationText", css.newUserPw), auiCtx);
